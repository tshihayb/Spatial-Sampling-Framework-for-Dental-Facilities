...
#
# Analysis of the distribution of private dental facilities in Riyadh, supporting the
# manuscript "Validating Dental Sampling Frames in Rapidly Expanding Cities: Riyadh Case"
# (JDR Clinical & Translational Research).
#
# Author:       Talal S. Alshihayb
# Created:      December 4, 2025
# Last updated: July 28, 2026
# Repository:   https://github.com/tshihayb/Spatial-Sampling-Framework-for-Dental-Facilities
# License:      Code MIT (see LICENSE); data CC BY 4.0 (see LICENSE-data.txt). See README.md.
#
# HOW TO RUN
#   1. Launch R with the working directory set to the repository root (open the project
#      in RStudio, or run setwd("/path/to/this/repo") once). No absolute path is hard-coded
#      below, so the script is portable across machines.
#   2. Packages used (install any that are missing): tidyverse, readxl, janitor, labelled,
#      irr, flextable, gtsummary, officer, scales, sf, ggrepel, RColorBrewer, viridis,
#      ragg, patchwork, ggnewscale, ggtext.
#   3. Run top to bottom.
#
# REPRODUCIBILITY NOTE (please read)
#   Under Saudi GASTAT census data-governance rules, district/neighbourhood population
#   counts cannot be shared publicly (only region/governorate totals may be published),
#   so they are removed from the public data. Therefore:
#     * Facility enumeration, validation, quality control, spatial joins, facility COUNTS,
#       and the kernel-density maps reproduce fully from the public data.
#     * The population-standardised steps (facilities per 10,000; Figure 3; Appendix
#       Figure 5 panel B; the per-10,000 regional means) require the restricted district-
#       population file, available from the authors on reasonable request. When the script
#       is run on the public data it stops with an explanatory message at that point.
#
# See README.md and DATA_DICTIONARY.md for full documentation.
#######################################################################################
...
###################################################
{
  ...
  # --- Working directory -------------------------------------------------------
  # Launch R from the repository root so the data files below resolve by relative
  # path. No absolute path is hard-coded here (kept portable). If you need to set it
  # for your machine, do it once, e.g.:
  #   setwd("/path/to/Spatial-Sampling-Framework-for-Dental-Facilities")
...

{
  ...
  names_dist <- read_excel("District_population_street_data_stripped.xlsx", sheet = "List") %>% 
    clean_names() %>% 
...
pop <- read_excel("District_population_street_data_stripped.xlsx", sheet = "New Riyadh Regions") %>%
  clean_names()

# --- Restricted-data guard (Saudi GASTAT census governance) -----------------------
# District/neighbourhood population counts cannot be shared publicly and are removed
# from the public data. Everything above reproduces from the public data; the
# population-standardised steps below require the restricted district-population file
# (available from the authors on reasonable request).
if (!"population" %in% names(pop)) {
  stop("District-level population is not included in the public data release under Saudi ",
       "GASTAT census data-governance rules (only region/governorate totals may be shared). ",
       "All facility-count analyses above run on the public data; the population-standardised ",
       "steps below require the restricted district-population file, available from the authors ",
       "on reasonable request.", call. = FALSE)
}
